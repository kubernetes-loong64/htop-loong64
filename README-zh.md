...
通过 CI/CD 构建 [htop](https://github.com/htop-dev/htop) 的 **LoongArch64 (loong64)** 架构二进制文件、RPM 和 DEB 包。

## 工作原理

GitHub Actions 工作流克隆指定的 htop 版本，通过 QEMU binfmt 仿真在 `linux/loong64` Docker 容器内
交叉编译，将构建的二进制文件打包为 RPM 和 DEB。目标平台：`linux/loong64`。

## 分支命名

推送 `loong64-<htop 版本>` 格式的分支（如 `loong64-3.5.3`）即可触发构建。可追加 `+<build>`
（如 `loong64-3.5.3+0`）携带构建元数据。

## [发布](https://github.com/kubernetes-loong64/htop-loong64/releases)

推送 `release-loong64-<htop 版本>` 格式的标签（如 `release-loong64-3.5.3+0`）即可自动创建 GitHub
Release 并上传构建产物。
...
|--------------------------------------------|----------------|
| `htop`                                     | htop 二进制文件     |
| `htop-<version>-<release>.loongarch64.rpm` | RPM 包 (Anolis) |
| `htop_<version>-<release>.loong64.deb`     | DEB 包 (Debian) |
